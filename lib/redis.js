...
/**
 * Limits the sorted set by the given limit count.
 *
 * @param {String} key
 * @param {Number} limit
 * @api public
...
Manager.prototype.zLimit = function (key, limit) {
    var self = this;
    this.client.zcard(key, function (err, count) {
        if (typeof count == 'number' && count > limit) {
            self.client.zremrangebyrank([key, limit, count], function (err, response) {});
        }
    });
};
