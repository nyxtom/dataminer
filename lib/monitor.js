...
WorkMonitor.prototype._initialize = function (options) {
    ...
    this.redisManager = new redis.Manager(options);
...
WorkMonitor.prototype._shutdown = function () {
    try {
...
WorkMonitor.prototype._report = function () {
    for (var i = 0; i < this.workers.length; i++) {
        ...
        if (typeof worker.count == 'number') {
            var current = worker.count;
            worker.count = 0;
            ...
            this.redisManager.zAdd("dm:workers:pps:" + worker.id, now.toString() + ":" + countPerSec.toString(), now);
            this.redisManager.zLimit("dm:workers:pps:" + worker.id, this.ppsZLimit);
        }
    }
};
