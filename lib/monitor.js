...
/**
 * Registers a worker with the given id to ensure that 
 * the state of the worker is maintained after the worker 
 * has process has ended.
 *
 * @param {Worker} worker
 * @param {String} id
 * @param {Object} options
...
exports.register = function (worker, id, options) {
    WorkMonitor.singleton.register(worker, id, options);
};

WorkMonitor.prototype.register = function (worker, id, options) {
    var self = this;
    ...
    this.redisManager.set("dm:workers:registered", worker.id, true);
};
...
exports.unregister = function (id, options) {
    WorkMonitor.singleton.unregister(id, options);
};

WorkMonitor.prototype.unregister = function (id, options) {
    var self = this;
    if (!this.redisManager) {
        this._initialize(options);
    }

    this.redisManager.hDel("dm:workers:registered", id);
    this.redisManager.del("dm:workers:pps:" + id);
};
...
WorkMonitor.prototype.shutdown = function () {
    try {
        ...
        for (var i = 0; i < this.workers.length; i++) {
            ...
            if (this.workers[i].registered) {
                this._reportWorker(this.workers[i]);
            }
            else {
...
WorkMonitor.prototype._report = function () {
    for (var i = 0; i < this.workers.length; i++) {
        ...
        this._reportWorker(worker);
    }
};

/**
 * Reports for a specific worker.
 *
 * @param {Worker} worker
 * @api private 
 */

WorkMonitor.prototype._reportWorker = function (worker) {
    var workStatus = {
        _id: worker.id,
        name: worker.name,
